...
#define AIRY_CONVOLUTION_TILE_SIZE 64   // Only used when AIRY_VIA_FFT is false
...
/*_______________ Airy Bloom ______________*/
#if AIRY_VIA_FFT
    ...
    bool apply_airy_bloom_internal(const ImageF *src, ImageF *dst, const double scale[3])
    {
        int W = src->width;
        int H = src->height;
...
bool apply_airy_bloom(const ImageF *src, ImageF *dst, const double scale[3])
{
    ...
    return apply_airy_bloom_internal(src, dst, scale);
}


/* _________________ Gaussian Blur __________________ */
