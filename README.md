...
* **Bowie (Binet) Integrator:** An alternative geodesic integrator that reduces each (planar) photon orbit to the Binet equation u'' = 3ku² − u and advances it with the explicit 4th-order Bowie single-step method (NASA, 1963), stepping by the azimuthal angle instead of the affine parameter. Stepping in φ is naturally adaptive in space (fine near the hole, coarse far away), conserves angular momentum exactly by construction, and lets escaped rays terminate themselves. Compared to the default RK4, it is at least 2× faster at equal quality and far more accurate for near-critical rays around the photon sphere.
